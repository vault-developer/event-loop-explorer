...
interface ProcessProps {
  ...
  animation: EventLoopAnimationState['mutable']
}

const processTask = async ({mutable, set, setAnimation, animation}: ProcessProps) => {
...
const processMicroTask = async ({mutable, set, setAnimation, animation}: ProcessProps) => {
...
const processRender = async ({mutable, set, setAnimation, animation}: ProcessProps) => {
...
export const useProcessEvent = () => {
  ...
  const animation = useEventLoopAnimationState(state => state.mutable);

  return useCallback(async (type: EventInterface['type']) => {
    if (type === 'task') {
      await processTask({mutable, set, setAnimation, animation});
    } else if (type === 'microtask') {
      await processMicroTask({mutable, set, setAnimation, animation});
    } else if (type === 'render') {
      await processRender({mutable, set, setAnimation, animation});
    }
  }, [mutable]);
};